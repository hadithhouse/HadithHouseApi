...
class PersonGetApiTestCase(TestCaseBase):
    def test__get_json__200(self):
        resp = self.get_person()
        self.assertEqual(HTTP_200_OK, resp.status_code)

    def test__get_form__200(self):
        resp = self.get_person(HTTP_ACCEPT='text/html')
        self.assertEqual(HTTP_200_OK, resp.status_code)
...
class PersonPutApiTestCase(TestCaseBase):
  ...
  def tearDownClass(cls):
    c = Client()
    resp = c.delete('/apis/persons/%d?fb_token=%s' % (PersonPutApiTestCase.person_id, TestCaseBase.marie_accesstoken),
                    {'title': 'test'})
...
class PersonPatchApiTestCase(TestCaseBase):
  person = None
  person_id = None
  ...
  def tearDownClass(cls):
    c = Client()
    resp = c.delete('/apis/persons/%d?fb_token=%s' % (PersonPatchApiTestCase.person_id, TestCaseBase.marie_accesstoken),
                    {'full_name': 'test'})
    assert resp.status_code == HTTP_204_NO_CONTENT
    TestCaseBase.tearDownClass()

  def test__patch__no_auth_token__403(self):
    resp = self.patch('/apis/persons/%d' % PersonPatchApiTestCase.person_id, {'full_name': 'test'})
    self.assertEqual(HTTP_403_FORBIDDEN, resp.status_code)
    self.assertEqual(HTTP_403_FORBIDDEN, resp.data['status_code'])
    self.assertEqual("Couldn't authenticate user.", resp.data['error'])

  def test__patch__invalid_auth_token__403(self):
    resp = self.patch('/apis/persons/%d?fb_token=%s' %
                      (PersonPatchApiTestCase.person_id, TestCaseBase.invalid_accesstoken), {'full_name': 'test'})
    self.assertEqual(HTTP_403_FORBIDDEN, resp.status_code)
    self.assertEqual(HTTP_403_FORBIDDEN, resp.data['status_code'])
    self.assertEqual("Invalid Facebook access token.", resp.data['error'])
  ...
  def test__patch__valid_auth_token__no_user_permission__401(self):
    resp = self.patch('/apis/persons/%d?fb_token=%s' %
                      (PersonPatchApiTestCase.person_id, TestCaseBase.jack_accesstoken), {'full_name': 'test'})
    self.assertEqual(HTTP_403_FORBIDDEN, resp.status_code)
    self.assertEqual(HTTP_403_FORBIDDEN, resp.data['status_code'])
    self.assertEqual("User doesn't have permission for this action.", resp.data['error'])
  ...
  def test__patch__valid_auth_token__user_permission__no_title__200(self):
    resp = self.patch('/apis/persons/%d?fb_token=%s' %
                      (PersonPatchApiTestCase.person_id, TestCaseBase.marie_accesstoken), {})
    self.assertEqual(HTTP_200_OK, resp.status_code)

  def test__patch__valid_auth_token__user_permission__blank_title__400(self):
    resp = self.patch('/apis/persons/%d?fb_token=%s' %
                      (PersonPatchApiTestCase.person_id, TestCaseBase.marie_accesstoken), {'full_name': ' '})
    self.assertEqual(HTTP_400_BAD_REQUEST, resp.status_code)
    self.assertEqual(HTTP_400_BAD_REQUEST, resp.data['status_code'])
    self.assertEqual("Invalid input.", resp.data['error'])
    self.assertTrue('full_name' in resp.data['detail'])
  ...
  def test__patch__valid_auth_token__user_permission__valid_title__person_updated(self):
    resp = self.patch('/apis/persons/%d?fb_token=%s' %
                      (PersonPatchApiTestCase.person_id, TestCaseBase.marie_accesstoken), {'full_name': 'test_updated'})
    self.assertEqual(HTTP_200_OK, resp.status_code)
    person = resp.data
    self.assertEqual('test_updated', person['full_name'])
