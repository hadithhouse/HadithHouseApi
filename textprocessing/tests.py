...
from textprocessing.generic import multiline_to_singleline, \
    remove_brackets_whitespaces, reformat_text, \
...
class ArabicTestCase(SimpleTestCase):
    def test_remove_arabic_diacritic(self):
        text = u'اخْتِبار ازَاْلة عَلامات التَشْكيل'
        output = remove_arabic_diacritics(text)
        self.assertEqual(u'اختبار ازالة علامات التشكيل', output)

    def test_unify_alef_letters(self):
        text = u'اآأإٱٲٳٵ'
        output = unify_alef_letters(text)
    ...
    def test_simplify_arabic_text_for_surat_alfatiha(self):
        self.assertEqual(u'بسم الله الرحمن الرحيم', simplify_arabic_text(
            u'بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ'))
        ...
        self.assertEqual(u'الرحمن الرحيم',
                         simplify_arabic_text(u'الرَّحْمَنِ الرَّحِيمِ'))
        self.assertEqual(u'مالك يوم الدين',
                         simplify_arabic_text(u'مَالِكِ يَوْمِ الدِّينِ'))
        self.assertEqual(u'اياك نعبد واياك نستعين', simplify_arabic_text(
            u'إِيَّاكَ نَعْبُدُ وَإِيَّاكَ نَسْتَعِينُ'))
        self.assertEqual(u'اهدنا الصراط المستقيم', simplify_arabic_text(
            u'اهدِنَا الصِّرَاطَ الْمُسْتَقِيمَ'))
        self.assertEqual(
