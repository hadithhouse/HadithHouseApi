...
class DocScanner(object):
    ...
    def __init__(self, tokens_dict, callback):
        ...
        self.scanning_regex = '|'.join(
            ['(?P<%s>%s)' % (type, regex) for type, regex in
             tokens_dict.items()])
    ...
    def scan(self, document, context=None):
        ...
        :param document: The document to start the scanning process on.
        :param context: A context to be sent to the call back. The caller is
        pass any thing to this parameter.
        :return:
        """
        prev_match = None
        prev_type = None
        for match in re.finditer(self.scanning_regex, document,
                                 flags=re.MULTILINE):
